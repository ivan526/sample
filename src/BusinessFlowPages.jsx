...
export function CollectionPlanPage({ products = [], stages = [], organizations = [], plans = [], onCreatePlan, onReleasePlan, onExportPlan, onOpenProgress, showToast }) {
  const [query, setQuery] = useState("");
  const [newPlanOpen, setNewPlanOpen] = useState(false);
  const [planForm, setPlanForm] = useState({ productId: products[0]?.id || "", stage: stages[0] || "", deadline: "2026-09-20T18:00", scope: "全部MSS区域" });
  ...
  const createPlan = async () => {
    ...
    if (!product || !planForm.stage) { showToast("请选择产品和样机阶段", "warning"); return; }
    try {
      await onCreatePlan({ productId: product.id, stage: planForm.stage, regionIds: organizations.filter((item) => item.enabled).map((item) => item.id), deadline: new Date(planForm.deadline).toISOString() });
