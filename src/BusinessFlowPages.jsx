...
import {
  ...
  IconFlag, IconHierarchy3, IconHistory, IconLink, IconMapPin, IconNotes, IconPackage, IconPlus,
...
export function DomainTaskPage({ products = [], organizations = [], plans = [], onDispatch, onOpenTask, showToast }) {
  ...
  const pendingChanges = domainPlans.reduce((sum, item) => sum + Number(item.pendingChangeCount || 0), 0);
  ...
  return <main className="workspace workspace-no-footer">
    ...
    <MetricStrip items={[
      ...
      { label: "待反馈/变更审批", value: feedbackPending + pendingChanges, unit: "项", hint: pendingChanges ? `${pendingChanges}个区域变更待审批` : "区域已全部收齐", tone: feedbackPending + pendingChanges ? "amber" : "blue", icon: IconClipboardCheck },
    ...
    {dispatchPlan && <Dialog wide title="配置并下发领域任务" description={`${dispatchPlan.mssDomain?.name} · ${dispatchProduct?.name} · ${dispatchPlan.stage}`} onClose={() => setDispatchPlan(null)} footer={<><button className="button button-secondary compact-button" type="button" onClick={() => setDispatchPlan(null)}>取消</button><button className="button button-primary compact-button" type="button" onClick={submitDispatch}><IconSend size={17} />下发给区域接口人</button></>}>
      <div className="dialog-form domain-dispatch-form">
        ...
        <fieldset className="scope-picker">
          ...
          <div>{organizations.filter((region) => region.enabled).map((region) => <label key={region.id} className={selectedRegionIds.includes(region.id) ? "scope-option-selected" : ""}><input type="checkbox" checked={selectedRegionIds.includes(region.id)} onChange={() => toggle(setSelectedRegionIds, region.id)} /><span><strong>{region.name}</strong></span></label>)}</div>
...
export function RegionalTaskPage({ products = [], organizations = [], plans = [], activeRegion, onOpenEntry }) {
  ...
  return <main className="workspace workspace-no-footer">
    ...
    <MetricStrip items={[
      { label: "待填报/修改中", value: visiblePlans.filter((plan) => plan.regionProgress?.find((item) => item.regionId === region.id)?.status !== "SUBMITTED").length, unit: "个", hint: "来自MSS领域接口人", tone: "amber", icon: IconClipboardCheck },
      { label: "已提交任务", value: visiblePlans.filter((plan) => plan.submittedRegions.includes(region.id)).length, unit: "个", hint: `${visiblePlans.filter((plan) => plan.regionProgress?.find((item) => item.regionId === region.id)?.changePending).length}个变更审批中`, icon: IconCircleCheckFilled },
    ...
    <section className="ops-surface domain-task-surface"><div className="surface-title"><div><h2>我的填报任务</h2><p>仅展示领域接口人已下发到当前区域的任务；提交后可按业务阶段撤回或申请变更</p></div><span className="surface-summary">共 <strong>{visiblePlans.length}</strong> 条任务</span></div><div className="plain-table-wrap"><table className="plain-table regional-task-table"><thead><tr><th>计划 / 产品</th><th>来源领域</th><th>本区域组织范围</th><th>本次产品型号</th><th>填报状态</th><th>截止时间</th><th>操作</th></tr></thead><tbody>{visiblePlans.map((plan) => { const product = products.find((item) => item.id === plan.productId); const progress = plan.regionProgress?.find((item) => item.regionId === region.id); const submitted = progress?.status === "SUBMITTED"; const pending = Boolean(progress?.changePending && progress?.changeRequest?.status === "PENDING"); const editing = ["DRAFT", "RETURNED"].includes(progress?.status); const statusLabel = pending ? "变更审批中" : submitted ? `已提交 V${progress?.revisionNo || 1}` : editing ? "修改中" : "待填报"; return <tr key={plan.viewId}><td><strong>{product?.name}</strong><small>{plan.planNo} · {plan.stage}</small></td><td><strong>{plan.mssDomain?.name || "待配置MSS领域"}</strong><small>领域接口人 {plan.mssDomain?.owner || "待配置"}</small></td><td><span className="org-count"><IconBuilding size={16} />{region.offices.length}个代表处</span><span className="org-count"><IconFlag size={16} />{countryCount}个国家/地区</span></td><td><strong>{plan.selectedSkuIds?.length || 1}项</strong><small>{product?.skus.length ? `领域从${product.skus.length}个型号中选择` : "可先按产品填报"}</small></td><td><span className={`status-badge ${submitted && !pending ? "badge-success" : "badge-warning"}`}>{statusLabel}</span>{progress?.reopenReason && !submitted && <small>原因：{progress.reopenReason}</small>}</td><td>{plan.deadline}</td><td><button className="table-action" type="button" onClick={() => onOpenEntry(plan.viewId, region.id)}><IconChevronRight size={15} />{submitted ? "查看与变更" : editing ? "继续修改" : "进入填报"}</button></td></tr>; })}</tbody></table></div></section>
  </main>;
}

export function CollectionTaskDetailPage({ role, plan, products = [], organizations = [], rowsByProduct = {}, initialTab = "progress", onBack, onOpenEntry, onOpenHistory, onFeedback, onDecideChange, showToast }) {
  ...
  const rawDemandItems = plan.demandItems?.length ? plan.demandItems : (plan.feedback?.items || []);
  const submittedDemandItems = rawDemandItems.map((item) => ({
    productSkuId: item.productSkuId || item.product_sku_id || "",
    provisionalItemKey: item.provisionalItemKey || item.provisional_item_key || "",
    model: item.model || item.skuModel || item.sku_model || item.provisionalItemKey || item.provisional_item_key || "型号待补充",
    bomCode: item.bomCode || item.bom_code || "",
    mssDomainId: item.mssDomainId || item.mss_domain_id || plan.mssDomain?.id || plan.mssDomainId || "",
    mssDomainName: item.mssDomainName || item.mss_domain_name || plan.mssDomain?.name || "待配置MSS领域",
    regionId: item.regionId || item.region_id || "",
    regionName: item.regionName || item.region_name || "待配置区域",
    quantity: Number(item.quantity) || 0,
  }));
  const summaryRegionMap = new Map();
  (plan.regionProgress || []).forEach((progress) => summaryRegionMap.set(progress.regionId, progress.regionName));
  submittedDemandItems.forEach((item) => { if (item.regionId) summaryRegionMap.set(item.regionId, item.regionName); });
  const summaryRegions = Array.from(summaryRegionMap, ([id, name]) => ({ id, name }));
  const summaryDomainDefinitions = (isGtm ? (plan.domainTasks || []).map((task) => ({
    id: task.mssDomainId,
    name: task.mssDomainName,
    selectedSkuIds: task.selectedSkuIds || [],
  })) : [{
    id: plan.mssDomain?.id || plan.mssDomainId || "current-domain",
    name: plan.mssDomain?.name || "待配置MSS领域",
    selectedSkuIds: plan.selectedSkuIds || [],
  }]).filter((domain) => domain.selectedSkuIds.length || submittedDemandItems.some((item) => item.mssDomainId === domain.id));
  const bomSummaryGroups = summaryDomainDefinitions.map((domain) => {
    const domainItems = submittedDemandItems.filter((item) => item.mssDomainId === domain.id);
    const configuredSkus = domain.selectedSkuIds.length
      ? product.skus.filter((sku) => domain.selectedSkuIds.includes(sku.id))
      : product.skus.filter((sku) => domainItems.some((item) => item.productSkuId === sku.id));
    const definitions = configuredSkus.map((sku) => ({ id: sku.id, model: sku.sku, bomCode: sku.bom, description: sku.description || "" }));
    domainItems.forEach((item) => {
      const known = definitions.some((sku) => item.productSkuId ? sku.id === item.productSkuId : sku.model === item.model && sku.bomCode === item.bomCode);
      if (!known) definitions.push({ id: item.productSkuId || item.provisionalItemKey || `${item.model}-${item.bomCode}`, model: item.model, bomCode: item.bomCode, description: "" });
    });
    const rows = definitions.map((sku) => {
      const items = domainItems.filter((item) => item.productSkuId ? item.productSkuId === sku.id : item.model === sku.model && item.bomCode === sku.bomCode);
      const quantities = Object.fromEntries(summaryRegions.map((region) => [region.id, items.filter((item) => item.regionId === region.id).reduce((sum, item) => sum + item.quantity, 0)]));
      return { ...sku, quantities, total: Object.values(quantities).reduce((sum, quantity) => sum + quantity, 0) };
    });
    const quantities = Object.fromEntries(summaryRegions.map((region) => [region.id, domainItems.filter((item) => item.regionId === region.id).reduce((sum, item) => sum + item.quantity, 0)]));
    return { ...domain, rows, quantities, total: domainItems.reduce((sum, item) => sum + item.quantity, 0) };
  });
  ...
  const openDecision = (progress, approved) => {
    setDecisionTarget(progress);
    setDecisionApproved(approved);
    setDecisionNote(approved ? "同意本次变更，重新开放区域填报" : "暂不同意变更，请补充变更范围及影响说明");
  };
  const submitDecision = async () => {
    if (!decisionNote.trim()) { showToast("请填写审批意见", "warning"); return; }
    setDecisionSubmitting(true);
    try {
      await onDecideChange(decisionTarget.changeRequest, decisionApproved, decisionNote.trim());
      showToast(decisionApproved ? "已通过变更申请，区域任务已恢复为修改中" : "已驳回变更申请");
      setDecisionTarget(null);
    } catch (error) { showToast(error.message || "变更审批失败", "warning"); }
  ...
  return <main className="workspace workspace-no-footer">
    ...
    <section className="ops-surface task-detail-surface"><div className="task-tabs" role="tablist" aria-label="收集任务详情"><button type="button" role="tab" aria-selected={activeTab === "progress"} className={activeTab === "progress" ? "task-tab-active" : ""} onClick={() => setActiveTab("progress")}><IconHierarchy3 size={18} />{isGtm ? "收集进度" : "区域收集进度"}</button><button type="button" role="tab" aria-selected={activeTab === "summary"} className={activeTab === "summary" ? "task-tab-active" : ""} onClick={() => setActiveTab("summary")}><IconFileSpreadsheet size={18} />领域需求汇总</button><button type="button" role="tab" aria-selected={activeTab === "feedback"} className={activeTab === "feedback" ? "task-tab-active" : ""} onClick={() => setActiveTab("feedback")}><IconSend size={18} />{isGtm ? "领域反馈" : "反馈GTM"}{!isGtm && plan.status === "待领域反馈" && <span className="task-tab-dot" />}</button></div>
      {activeTab === "progress" && <div className="task-tab-panel"><div className="surface-title compact-surface-title"><div><h2>{isGtm ? "MSS领域任务进度" : "区域—代表处—国家收集进度"}</h2><p>{isGtm ? "查看各领域的二次下发、区域收集和反馈状态" : "点击区域进入收集；有变更申请时由领域接口人在此审批"}</p></div><span className="scope-path"><IconWorld size={17} />组织口径已同步</span></div>{isGtm ? <div className="plain-table-wrap"><table className="plain-table region-progress-table"><thead><tr><th>MSS业务领域</th><th>领域接口人</th><th>已选型号</th><th>区域进度</th><th>需求汇总</th><th>任务状态</th></tr></thead><tbody>{(plan.domainTasks || []).map((task) => <tr key={task.id}><td><strong>{task.mssDomainName}</strong><small>{task.mssDomainId}</small></td><td>{task.owner}</td><td><strong>{task.selectedSkuIds.length}个</strong><small>{task.status === "PENDING_DISPATCH" ? "等待领域选择" : "已完成范围配置"}</small></td><td><span className="plan-progress"><span><i style={{ width: `${task.submittedRegions / Math.max(1, task.totalRegions) * 100}%` }} /></span><strong>{task.submittedRegions}/{task.totalRegions}</strong></span></td><td><strong>{task.demandTotal.toLocaleString()} Pcs</strong></td><td><span className={`status-badge ${task.status === "FEEDBACK_SUBMITTED" ? "badge-success" : task.status === "PENDING_DISPATCH" ? "badge-warning" : "badge-info"}`}>{({ PENDING_DISPATCH: "待下发区域", COLLECTING: "区域收集中", READY_TO_FEEDBACK: "待反馈GTM", FEEDBACK_SUBMITTED: "已反馈GTM" })[task.status]}</span></td></tr>)}</tbody></table></div> : <div className="plain-table-wrap"><table className="plain-table region-progress-table"><thead><tr><th>MKT区域</th><th>区域接口人</th><th>代表处 / 国家</th><th>区域需求</th><th>提交 / 变更状态</th><th>领域操作</th></tr></thead><tbody>{taskOrganizations.map((region) => { const progress = plan.regionProgress?.find((item) => item.regionId === region.id); const submitted = progress?.status === "SUBMITTED"; const pending = Boolean(progress?.changePending && progress?.changeRequest?.status === "PENDING"); const officeCount = region.offices?.length || 0; const countryCount = (region.offices || []).reduce((sum, office) => sum + (office.countries?.length || 0), 0); const statusLabel = pending ? "变更待审批" : submitted ? `已提交 V${progress?.revisionNo || 1}` : ["DRAFT", "RETURNED"].includes(progress?.status) ? "修改中" : "未开始"; return <tr key={region.id} className={pending ? "pending-change-row" : ""}><td><strong>{region.name}</strong><small>{region.id.toUpperCase()}</small></td><td><strong>{region.owner || "待配置"}</strong><small>区域接口人</small></td><td><span className="org-count"><IconBuilding size={16} />{officeCount}个代表处</span><span className="org-count"><IconFlag size={16} />{countryCount}个国家/地区</span></td><td><strong>{regionDemand(region.id).toLocaleString()} Pcs</strong><small>{skuUnits.length}个产品项</small></td><td><span className={`status-badge ${pending ? "badge-warning" : submitted ? "badge-success" : regionDemand(region.id) ? "badge-warning" : ""}`}>{statusLabel}</span>{pending && <small className="change-reason-preview">原因：{progress.changeRequest.reason}</small>}{!pending && progress?.reopenReason && !submitted && <small className="change-reason-preview">已开放：{progress.reopenReason}</small>}</td><td><div className="table-action-group"><button className="table-action" type="button" onClick={() => onOpenEntry(plan.viewId, region.id)}><IconChevronRight size={15} />{submitted ? "查看" : "进入收集"}</button>{Number(progress?.revisionNo || 0) > 0 && <button className="table-action" type="button" onClick={() => onOpenHistory(plan, region.id)}><IconHistory size={15} />版本历史</button>}{pending && <><button className="table-action action-approve" type="button" onClick={() => openDecision(progress, true)}><IconCheck size={15} />通过</button><button className="table-action action-reject" type="button" onClick={() => openDecision(progress, false)}><IconX size={15} />驳回</button></>}</div></td></tr>; })}</tbody></table></div>}</div>}
      {activeTab === "summary" && <div className="task-tab-panel"><div className="surface-title compact-surface-title"><div><h2>领域需求汇总</h2><p>按MSS领域、产品型号和BOM逐级汇总已提交的区域需求</p></div><span className={`summary-readiness ${allSubmitted ? "summary-ready" : "summary-pending"}`}>{allSubmitted ? <IconCircleCheckFilled size={17} /> : <IconClockHour4 size={17} />}{allSubmitted ? "区域已全部收齐" : `还有${Math.max(0, plan.total - submittedCount)}个区域待提交`}</span></div><div className="plain-table-wrap"><table className="plain-table domain-summary-table"><thead><tr><th>MSS领域 / 产品型号</th><th>BOM编码</th>{summaryRegions.map((region) => <th key={region.id}>{region.name.replace("MKT", "")}</th>)}<th>领域合计</th></tr></thead><tbody>{bomSummaryGroups.length ? bomSummaryGroups.map((group) => [<tr className="domain-product-row" key={`${group.id}-total`}><td><strong>{group.name}</strong><small>{product?.name} · {plan.stage}</small></td><td>{group.rows.length ? `${group.rows.length}个BOM` : "待补充"}</td>{summaryRegions.map((region) => <td key={region.id}><strong>{Number(group.quantities[region.id] || 0).toLocaleString()}</strong></td>)}<td><strong>{group.total.toLocaleString()} Pcs</strong></td></tr>, ...group.rows.map((sku) => <tr key={`${group.id}-${sku.id}`}><td><span className="sku-indent">{sku.model}</span>{sku.description && <small className="sku-description">{sku.description}</small>}</td><td>{sku.bomCode || <span className="bom-placeholder">待补充</span>}</td>{summaryRegions.map((region) => <td key={region.id}>{Number(sku.quantities[region.id] || 0).toLocaleString()}</td>)}<td><strong>{sku.total.toLocaleString()}</strong></td></tr>)]) : <tr><td className="table-empty" colSpan={summaryRegions.length + 3}>暂无已提交的BOM反馈明细；区域提交需求后将在这里自动汇总。</td></tr>}</tbody></table></div></div>}
    ...
    {decisionTarget && <Dialog title={`${decisionApproved ? "通过" : "驳回"}区域变更申请`} description={`${decisionTarget.regionName} · 当前提交V${decisionTarget.revisionNo || 1}`} onClose={() => !decisionSubmitting && setDecisionTarget(null)} footer={<><button className="button button-secondary compact-button" type="button" disabled={decisionSubmitting} onClick={() => setDecisionTarget(null)}>取消</button><button className={`button ${decisionApproved ? "button-primary" : "button-danger"} compact-button`} type="button" disabled={decisionSubmitting || !decisionNote.trim()} onClick={submitDecision}>{decisionSubmitting ? "处理中…" : `确认${decisionApproved ? "通过" : "驳回"}`}</button></>}><div className="change-approval-detail"><div><span>申请类型</span><strong>{({ REOPEN_REQUEST: "截止后重新开放", CHANGE_REQUEST: "领域反馈后变更", POST_EXPORT_CHANGE: "排产导出后变更" })[decisionTarget.changeRequest.type] || "区域变更"}</strong></div><div><span>区域需求</span><strong>{Number(decisionTarget.demand || 0).toLocaleString()} Pcs</strong></div><div className="change-approval-reason"><span>申请原因</span><strong>{decisionTarget.changeRequest.reason}</strong></div></div>{decisionApproved && <div className="change-request-warning"><IconAlertTriangleFilled size={20} /><div><strong>通过后将回退本领域收集进度</strong><span>如已反馈GTM，旧反馈会转入历史并失效；如已导出，历史文件继续保留，后续生成新版本。</span></div></div>}<label className="change-reason-field"><span>审批意见<sup>*</sup></span><textarea maxLength="500" value={decisionNote} onChange={(event) => setDecisionNote(event.target.value)} /><small>{decisionNote.length}/500</small></label></Dialog>}
  </main>;
}
