...
export function CollectionPlanPage({ products = [], stages = [], mssDomains = [], organizations = [], plans = [], onCreatePlan, onReleasePlan, onExportPlan, onOpenProgress, showToast }) {
  ...
  const [planForm, setPlanForm] = useState({ productId: products[0]?.id || "", stage: stages[0] || "", mssDomainId: mssDomains[0]?.id || "", deadline: "2026-09-20T18:00", scope: "全部MSS区域" });
  ...
  useEffect(() => { if (!planForm.mssDomainId && mssDomains[0]?.id) setPlanForm((current) => ({ ...current, mssDomainId: mssDomains[0].id })); }, [mssDomains, planForm.mssDomainId]);
  ...
  const createPlan = async () => {
    ...
    if (!product || !planForm.stage || !planForm.mssDomainId) { showToast("请选择产品、样机阶段和MSS业务领域", "warning"); return; }
    try {
      await onCreatePlan({ productId: product.id, stage: planForm.stage, mssDomainId: planForm.mssDomainId, regionIds: organizations.filter((item) => item.enabled).map((item) => item.id), deadline: new Date(planForm.deadline).toISOString() });
...
export function DomainTaskPage({ products = [], organizations = [], plans = [], onOpenTask }) {
  ...
  return <main className="workspace workspace-no-footer">
    <PageHeader title="需求收集 · 我的领域任务" description="MSS领域接口人工作台：组织区域收集、检查领域汇总，并将完整结果反馈给GTM" action={<span className="workbench-badge"><IconHierarchy3 size={17} />{assignedMssDomain?.name || "当前MSS领域"} · 接口人</span>} />
    <MetricStrip items={[
