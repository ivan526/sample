...
export function TsmpImportPanel({ onImported, showToast }) {
  ...
  return <><section className="ops-surface tsmp-import-surface"><div className="surface-title"><div><h2>TSMP发货数据匹配</h2><p>按MSS领域、区域、代表处、国家和产品BOM编码关联已确认需求</p></div><button className="button button-primary compact-button" type="button" onClick={() => setOpen(true)}><IconUpload size={17} />导入TSMP发货数据</button></div><div className="import-overview"><div className="import-file"><IconFileSpreadsheet size={25} /><span><strong>{fileName}</strong><small>最近导入：{importedAt} · 共{lastImport?.totalRows || 0}条</small></span></div><div className="import-rule"><span>自动匹配</span><strong>MSS领域</strong><IconPlus size={14} /><strong>产品BOM</strong><IconPlus size={14} /><strong>组织范围</strong></div><div className="import-numbers"><span><strong>{lastImport?.matchedRows || 0}</strong><small>自动匹配</small></span><span><strong>{lastImport?.mappingRequiredRows || 0}</strong><small>待维护映射</small></span><span className="warning-text"><strong>{lastImport?.unmatchedRows || 0}</strong><small>未匹配</small></span></div></div></section>{open && <Dialog wide title="导入TSMP发货数据" description="上传TSMP导出的Excel文件，系统将按下列正式字段关联需求" onClose={() => setOpen(false)} footer={<><button className="button button-secondary compact-button" type="button" onClick={() => setOpen(false)}>取消</button><button className="button button-primary compact-button" type="button" disabled={importing || !file} onClick={doImport}><IconDatabaseImport size={17} />{importing ? "正在导入…" : "开始导入匹配"}</button></>}><label className="import-dropzone"><IconUpload size={30} /><strong>{file ? `已选择 ${file.name}` : "选择TSMP导出的Excel文件"}</strong><span>支持 .xlsx / .xls，必需列：业务领域、地区部、代表处、国家/地区、BOM编码、发货数量</span><input type="file" accept=".xlsx,.xls" onChange={(event) => setFile(event.target.files?.[0] || null)} /></label><div className="import-field-map"><span>TSMP字段对应关系</span><div><strong>业务领域</strong><IconArrowRight size={16} /><i>MSS领域</i></div><div><strong>地区部</strong><IconArrowRight size={16} /><i>区域</i></div><div><strong>代表处</strong><IconArrowRight size={16} /><i>代表处</i></div><div><strong>国家/地区</strong><IconArrowRight size={16} /><i>国家</i></div><div><strong>BOM编码</strong><IconArrowRight size={16} /><i>产品BOM编码</i></div><div><strong>发货数量</strong><IconArrowRight size={16} /><i>实际发货数量</i></div></div></Dialog>}</>;
}
