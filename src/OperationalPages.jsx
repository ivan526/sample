...
const emptyProduct = (domains, mssDomains) => ({ id: `product-${Date.now()}`, name: "", categoryId: domains[0]?.id || "", mssDomainId: mssDomains[0]?.id || "mss-mkt", stage: "工程样机（EVT）", supply: "待产品线确认", deadline: "待计划下发", scope: "6个MKT区域", enabled: true, skus: [{ _uid: `sku-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, sku: "", bom: "", description: "" }] });
...
export function ConfigurationPage({ products, domains, mssDomains = [], organizations, dictionaries = {}, users = [], canEdit = true, canManageUsers = false, onAddProduct, onUpdateProduct, onAddDomain, onUpdateDomain, onAddOrganization, onUpdateOrganization, onAddDictionaryItem, onUpdateDictionaryItem, onDeleteDictionaryItem, onAddUser, onUpdateUser }) {
  ...
  const tabAction = activeTab === "products" ? { label: "新增产品", onClick: () => openProduct(null) }
    : activeTab === "domains" ? { label: "新增品类", onClick: () => openDomain(null) }
    : activeTab === "mss-domains" ? { label: "暂不支持新增", onClick: () => {} }
  ...
  return <main className="workspace workspace-no-footer">
    ...
    <section className="ops-surface config-surface">
      ...
      {activeTab === "mss-domains" && <div className="plain-table-wrap"><table className="plain-table config-table domain-config-table"><thead><tr><th>MSS业务领域</th><th>领域编码</th><th>领域说明</th><th>MSS接口人</th><th>关联产品</th><th>状态</th></tr></thead><tbody>{mssDomains.map((item) => { const linked = products.filter((product) => product.mssDomainId === item.id); return <tr key={item.id}><td><strong>{item.name}</strong><small>{item.id}</small></td><td><code>{item.code}</code></td><td>{item.description || "—"}</td><td><span className="owner-cell"><i>{item.mssOwner?.slice(0, 1) || "?"}</i><span><strong>{item.mssOwner || "待配置"}</strong><small>需求审核负责人（跨品类）</small></span></span></td><td><strong>{linked.length}个产品</strong><small>{linked.map((product) => product.name).join("、") || "暂无关联"}</small></td><td><StatusBadge>{item.enabled ? "启用中" : "已停用"}</StatusBadge></td></tr>; })}{!mssDomains.length && <tr><td className="empty-cell" colSpan={6}>未找到匹配的MSS领域配置</td></tr>}</tbody></table></div>}
