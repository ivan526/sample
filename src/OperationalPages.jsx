...
const emptyProduct = (domains) => ({ id: `product-${Date.now()}`, name: "", categoryId: domains[0]?.id || "", stage: "工程样机（EVT）", supply: "待产品线确认", deadline: "待计划下发", scope: "6个MKT区域", enabled: true, skus: [{ sku: "", bom: "" }] });
...
export function ConfigurationPage({ products, domains, organizations, dictionaries = {}, users = [], canEdit = true, canManageUsers = false, onAddProduct, onUpdateProduct, onAddDomain, onUpdateDomain, onAddOrganization, onUpdateOrganization, onAddDictionaryItem, onUpdateDictionaryItem, onDeleteDictionaryItem, onAddUser, onUpdateUser }) {
  ...
  const openProduct = (product) => { setProductForm(product ? { ...product, skus: product.skus.map((sku) => ({ ...sku })) } : emptyProduct(domains)); setEditingProduct(product?.id || "new"); setProductError(""); };
  ...
  const saveProduct = () => {
    const validSkus = productForm.skus.filter((sku) => sku.sku.trim()).map((sku) => ({ ...sku, sku: sku.sku.trim(), bom: sku.bom.trim() }));
