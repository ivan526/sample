...
export function ExecutionPage({ products, organizations, showToast, permissions = [] }) {
...
const emptyProduct = (domains) => ({ id: `product-${Date.now()}`, name: "", categoryId: domains[0]?.id || "", stage: "工程样机（EVT）", supply: "待产品线确认", deadline: "待计划下发", scope: "6个MKT区域", enabled: true, skus: [{ _uid: `sku-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, sku: "", bom: "", description: "" }] });
const emptyDomain = () => ({ id: `domain-${Date.now()}`, name: "", gtm: "", domainOwner: "", stockingOwner: "", description: "", enabled: true });
...
// 字典类型映射
const DICT_TYPE_MAP = {
  SAMPLE_STAGE: { name: "样机阶段", description: "产品所处的样机研发阶段" },
...
export function ConfigurationPage({ products, domains, organizations, dictionaries = {}, users = [], canEdit = true, canManageUsers = false, onAddProduct, onUpdateProduct, onAddDomain, onUpdateDomain, onAddOrganization, onUpdateOrganization, onAddDictionaryItem, onUpdateDictionaryItem, onDeleteDictionaryItem, onAddUser, onUpdateUser }) {
  ...
  const saveDomain = () => {
    if (!domainForm.name.trim() || !domainForm.gtm.trim() || !domainForm.domainOwner.trim() || !domainForm.stockingOwner.trim()) { setDomainError("请完整填写领域名称、GTM接口人、MSS领域接口人和备货接口人。"); return; }
    const payload = { ...domainForm, name: domainForm.name.trim(), gtm: domainForm.gtm.trim(), domainOwner: domainForm.domainOwner.trim(), stockingOwner: domainForm.stockingOwner.trim() };
  ...
  const tabAction = activeTab === "products" ? { label: "新增产品", onClick: () => openProduct(null) }
    : activeTab === "domains" ? { label: "新增领域", onClick: () => openDomain(null) }
    : activeTab === "dictionaries" ? { label: "新增字典项", onClick: () => openDictItem(null) }
    : activeTab === "organizations" ? { label: "新增区域", onClick: () => openRegion(null) }
    : { label: "新增用户", onClick: () => openUser(null) };
  const searchPlaceholder = activeTab === "products" ? "搜索产品、领域、责任人、SKU或BOM"
    : activeTab === "domains" ? "搜索领域或责任人"
  ...
  // 动态样机阶段选项
  const stageOptions = (dictionaries.SAMPLE_STAGE || [{ name: "工程样机（EVT）" }, { name: "测试样机（DVT）" }, { name: "测试样机（VN2）" }]).map(item => item.name);
  return <main className="workspace workspace-no-footer">
    <PageHeader title="配置管理" description="维护产品领域、责任人、组织架构及基础枚举数据，确保需求、执行和库存使用同一口径" action={canEdit ? <button className="button button-primary compact-button add-product-button" type="button" onClick={tabAction.onClick}><IconPlus size={18} />{tabAction.label}</button> : <span className="readonly-hint">只读模式</span>} />
    ...
    <div className="config-tabs" role="tablist" aria-label="配置类型">
      {[
        ...
        ["domains", "领域与责任人", domains.length],
        ["dictionaries", "基础数据", dictTotal],
        ...(canManageUsers ? [["users", "用户管理", users.length]] : []),
        ["organizations", "区域与代表处", officeTotal]
      ].map(([id, label, count]) => <button type="button" role="tab" aria-selected={activeTab === id} className={activeTab === id ? "config-tab-active" : ""} key={id} onClick={() => { setActiveTab(id); setQuery(""); }}>{label}<span>{count}</span></button>)}
    </div>
    <section className="ops-surface config-surface">
      <div className="surface-title"><div><h2>{activeTab === "products" ? "产品主数据" : activeTab === "domains" ? "领域与责任人" : activeTab === "dictionaries" ? "基础枚举数据" : activeTab === "users" ? "系统用户管理" : "区域与代表处"}</h2><p>{activeTab === "products" ? "产品选择所属领域后，自动继承该领域的GTM和备货接口人" : activeTab === "domains" ? "同一产品领域由固定GTM和领域备货接口人负责" : activeTab === "dictionaries" ? "维护样机阶段、MSS领域、需求依据等系统枚举值，全平台统一使用" : activeTab === "users" ? "维护系统各角色用户账号，配置后可分配到对应领域和区域" : "维护区域、代表处及国家/地区的组织层级和接口人"}</p></div><span className="surface-summary">{activeTab === "products" ? `共 ${visibleProducts.length} 个产品` : activeTab === "domains" ? `共 ${visibleDomains.length} 个领域` : activeTab === "dictionaries" ? `共 ${dictTotal} 个配置项` : activeTab === "users" ? `共 ${visibleUsers.length} 个用户` : `共 ${visibleOrganizations.length} 个区域 / ${officeTotal}个代表处`}</span></div>
      ...
      {activeTab === "products" && <div className="plain-table-wrap"><table className="plain-table config-table product-config-table"><thead><tr><th>产品名称</th><th>所属领域</th><th>样机阶段</th><th>样机提供时间</th><th>责任人</th><th>型号 / BOM</th><th>默认截止</th><th>状态</th>{canEdit && <th>操作</th>}</tr></thead><tbody>{visibleProducts.map((item) => { const domain = domainFor(item); const missingBom = !item.skus.length || item.skus.some((sku) => !sku.bom); return <tr key={item.id}><td><strong>{item.name}</strong><small>{item.id}</small></td><td><span className="domain-chip">{domain?.name || "待配置"}</span></td><td>{item.stage}</td><td>{item.supply || "待产品线确认"}</td><td><strong>GTM · {domain?.gtm || "待配置"}</strong><small>领域 · {domain?.domainOwner || "待配置"}</small><small>备货 · {domain?.stockingOwner || "待配置"}</small></td><td><strong>{item.skus.length ? `${item.skus.length}个型号` : "产品立项阶段"}</strong><small className={missingBom ? "warning-text" : ""}>
  {missingBom ? "BOM待产品线补充" : item.skus.map((sku) => (
    <div key={sku._uid || sku.sku} style={{marginTop: '3px', lineHeight: '1.4'}}>
      <span>{sku.sku}</span>{sku.bom ? <span style={{opacity: 0.8}}> / {sku.bom}</span> : null}
      {sku.description ? <div style={{color: '#6b7280', fontSize: '11px', marginTop: '1px'}}>{sku.description}</div> : null}
    </div>
  ))}
      ...
      {activeTab === "domains" && <div className="plain-table-wrap"><table className="plain-table config-table domain-config-table"><thead><tr><th>产品领域</th><th>领域说明</th><th>GTM接口人</th><th>MSS领域接口人</th><th>备货接口人</th><th>关联产品</th><th>状态</th>{canEdit && <th>操作</th>}</tr></thead><tbody>{visibleDomains.map((item) => { const linked = products.filter((product) => product.categoryId === item.id); return <tr key={item.id}><td><strong>{item.name}</strong><small>{item.id}</small></td><td>{item.description || "—"}</td><td><span className="owner-cell"><i>{item.gtm?.slice(0, 1) || "?"}</i><span><strong>{item.gtm || "待配置"}</strong><small>GTM负责人</small></span></span></td><td><span className="owner-cell"><i>{item.domainOwner?.slice(0, 1) || "?"}</i><span><strong>{item.domainOwner || "待配置"}</strong><small>需求收集负责人</small></span></span></td><td><span className="owner-cell"><i>{item.stockingOwner?.slice(0, 1) || "?"}</i><span><strong>{item.stockingOwner || "待配置"}</strong><small>备货执行衔接</small></span></span></td><td><strong>{linked.length}个产品</strong><small>{linked.map((product) => product.name).join("、") || "暂无关联"}</small></td><td><StatusBadge>{item.enabled ? "启用中" : "已停用"}</StatusBadge></td>{canEdit && <td><div className="config-actions"><button className="table-action" type="button" onClick={() => openDomain(item)}><IconPencil size={15} />编辑</button><button className="table-action muted-action" type="button" onClick={() => onUpdateDomain({ ...item, enabled: !item.enabled })}>{item.enabled ? "停用" : "启用"}</button></div></td>}</tr>; })}{!visibleDomains.length && <tr><td className="empty-cell" colSpan={canEdit ? 8 : 7}>未找到匹配的领域配置</td></tr>}</tbody></table></div>}
    ...
    {editingProduct && <Dialog wide title={editingProduct === "new" ? "新增新品项目" : "编辑产品配置"} description="新品启动时只需维护产品名称和所属领域，产品型号与BOM编码可在产品线确认后补充" onClose={() => setEditingProduct(null)} footer={<><button className="button button-secondary compact-button" type="button" onClick={() => setEditingProduct(null)}>取消</button><button className="button button-primary compact-button" type="button" onClick={saveProduct}>{editingProduct === "new" ? "创建产品" : "保存更新"}</button></>}><div className="config-form"><div className="config-form-grid"><label>产品名称<sup>*</sup><input value={productForm.name} onChange={(event) => updateProductField("name", event.target.value)} placeholder="例如 Chitu B23新品项目" /></label><label>所属领域<sup>*</sup><select value={productForm.categoryId} onChange={(event) => updateProductField("categoryId", event.target.value)} aria-label="所属领域"><option value="">请选择产品领域</option>{domains.filter((item) => item.enabled || item.id === productForm.categoryId).map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label>当前样机阶段<select value={productForm.stage} onChange={(event) => updateProductField("stage", event.target.value)}>{stageOptions.map(stage => <option key={stage} value={stage}>{stage}</option>)}</select></label><label>预计样机提供时间（可选）<input value={productForm.supply} onChange={(event) => updateProductField("supply", event.target.value)} placeholder="待产品线确认" /></label><label>计划默认截止（可选）<input value={productForm.deadline} onChange={(event) => updateProductField("deadline", event.target.value)} placeholder="在收集计划中设置" /></label><label>默认收集范围<select value={productForm.scope} onChange={(event) => updateProductField("scope", event.target.value)}>{organizations.map((_, index) => <option key={index}>{organizations.length - index}个MKT区域</option>)}<option>指定区域</option></select></label></div>{activeDomain && <div className="responsibility-preview"><span>领域责任人自动带出</span><div><strong>{activeDomain.name}</strong><i>GTM · {activeDomain.gtm}</i><i>MSS领域接口人 · {activeDomain.domainOwner || "待配置"}</i><i>备货接口人 · {activeDomain.stockingOwner}</i></div></div>}<div className="sku-builder"><div><span><h3>产品型号与BOM</h3><small>均可后补；有型号但暂无BOM时，只填写型号即可</small></span><button className="text-button" type="button" onClick={() => setProductForm((current) => ({ ...current, skus: [...current.skus, { _uid: `sku-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, sku: "", bom: "", description: "" }] }))}><IconPlus size={16} />添加型号</button></div>{productForm.skus.map((sku, index) => <div className="sku-builder-row" key={sku._uid}>
  <div style={{display: 'flex', gap: '8px', alignItems: 'center', width: '100%'}}>
    <span>{index + 1}</span>
    <input value={sku.sku} onChange={(event) => updateSku(index, "sku", event.target.value)} placeholder="产品型号（可后补）" aria-label={`第${index + 1}个产品型号`} style={{flex: '1 1 160px'}} />
    <input value={sku.bom} onChange={(event) => updateSku(index, "bom", event.target.value)} placeholder="BOM编码（可后补）" aria-label={`第${index + 1}个BOM编码`} style={{flex: '1 1 160px'}} />
    <button type="button" onClick={() => removeSku(index)} disabled={productForm.skus.length === 1} aria-label={`删除第${index + 1}个产品型号`} style={{flexShrink: 0}}><IconX size={17} /></button>
  </div>
  <input className="sku-desc-input" value={sku.description} onChange={(event) => updateSku(index, "description", event.target.value)} placeholder="产品描述（配置/规格说明，可选）" aria-label={`第${index + 1}个产品描述`} style={{width: '100%', marginTop: '6px', marginLeft: '28px'}} />
</div>)}</div>{productError && <p className="config-error"><IconAlertTriangleFilled size={16} />{productError}</p>}</div></Dialog>}
    {editingDomain && <Dialog title={editingDomain === "new" ? "新增产品领域" : "编辑领域与责任人"} description="同一领域下的产品自动继承这里配置的三类责任人" onClose={() => setEditingDomain(null)} footer={<><button className="button button-secondary compact-button" type="button" onClick={() => setEditingDomain(null)}>取消</button><button className="button button-primary compact-button" type="button" onClick={saveDomain}>{editingDomain === "new" ? "创建领域" : "保存更新"}</button></>}><div className="dialog-form"><label>领域名称<input value={domainForm.name} onChange={(event) => setDomainForm((current) => ({ ...current, name: event.target.value }))} placeholder="例如 PC" /></label><label>领域说明<input value={domainForm.description} onChange={(event) => setDomainForm((current) => ({ ...current, description: event.target.value }))} placeholder="填写该领域覆盖的产品类型" /></label><div className="two-field-row"><label>GTM接口人<sup>*</sup><input value={domainForm.gtm} onChange={(event) => setDomainForm((current) => ({ ...current, gtm: event.target.value }))} placeholder="请输入姓名" /></label><label>MSS领域接口人<sup>*</sup><input value={domainForm.domainOwner || ''} onChange={(event) => setDomainForm((current) => ({ ...current, domainOwner: event.target.value }))} placeholder="负责需求收集和审核" /></label></div><div className="two-field-row"><label>备货接口人<sup>*</sup><input value={domainForm.stockingOwner} onChange={(event) => setDomainForm((current) => ({ ...current, stockingOwner: event.target.value }))} placeholder="负责备货发货衔接" /></label></div>{domainError && <p className="config-error"><IconAlertTriangleFilled size={16} />{domainError}</p>}</div></Dialog>}
