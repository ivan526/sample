...
import {
  ...
  IconSettings, IconShieldCheck, IconTrash, IconTruckDelivery, IconUsers, IconWorld, IconX,
...
export function ExecutionPage({ products, organizations, showToast, permissions = [], currentUser }) {
  ...
  const exportCsv = () => {
    const lines = [["产品", "SKU", "BOM", "MSS领域", "区域", "代表处", "国家/地区", "确认需求", "已备货", "累计申请", "累计发货", "可用库存", "发货批次"], ...detailRows.map((row) => [row.productName, row.sku, row.bom, row.mssDomain, row.region, row.office, row.country, row.demand, row.stocked, row.applied, row.shipped, row.inventory, row.shipmentCount])];
...
const emptyOffice = () => ({ id: "", name: "", owner: "", countries: "" });
...
export function ConfigurationPage({ products = [], domains = [], mssDomains = [], organizations = [], dictionaries = {}, users = [], currentUserRole = 'ADMIN', canEdit: canEditProducts = true, canManageMss = false, canManageUsers = false, onAddProduct, onUpdateProduct, onAddDomain, onUpdateDomain, onAddMssDomain, onUpdateMssDomain, onAddOrganization, onUpdateOrganization, onDeleteOrganization, onAddDictionaryItem, onUpdateDictionaryItem, onDeleteDictionaryItem, onAddUser, onUpdateUser }) {
  ...
  const openRegion = (region) => { setOrganizationModal({ type: "region", id: region?.id || "new" }); setOrganizationForm(region ? { ...region, owner: region.owner === "待配置" ? "" : region.owner, offices: region.offices.map((office) => ({ ...office, countries: [...office.countries] })) } : emptyRegion()); setOrganizationError(""); };
  const openOffice = (region, office) => { setOrganizationModal({ type: "office", regionId: region.id, id: office?.id || "new" }); setOrganizationForm(office ? { ...office, owner: office.owner === "待配置" ? "" : office.owner, countries: office.countries.join("、") } : emptyOffice()); setOrganizationError(""); };
  const deleteRegion = (region) => {
    if (!window.confirm(`确认删除区域“${region.name}”？\n\n未被业务数据引用时将直接删除；已有历史业务数据时将停用并保留历史记录。`)) return;
    onDeleteOrganization(region);
  };
  const saveOrganization = () => {
    if (!organizationForm.name.trim()) { setOrganizationError("请填写名称。"); return; }
  ...
  return <main className="workspace workspace-no-footer">
    ...
    <section className="ops-surface config-surface">
      <div className="surface-title"><div><h2>{activeTab === "products" ? "产品主数据" : activeTab === "domains" ? "产品品类（GTM/备货）" : activeTab === "mss-domains" ? "MSS业务领域" : activeTab === "dictionaries" ? "基础枚举数据" : activeTab === "users" ? "系统用户管理" : "区域与代表处"}</h2><p>{activeTab === "products" ? "产品选择所属品类后，自动继承该品类的GTM和备货接口人" : activeTab === "domains" ? "同一产品品类由固定GTM和备货接口人负责" : activeTab === "mss-domains" ? "MSS领域负责人按业务线划分权限，可跨产品品类查看负责领域下的所有产品需求" : activeTab === "dictionaries" ? "维护样机阶段、需求依据等系统枚举值；MSS业务领域在独立页签维护" : activeTab === "users" ? "维护系统各角色用户账号，配置后可分配到对应领域和区域" : "维护区域、代表处及国家/地区的组织层级和接口人"}</p></div><span className="surface-summary">{activeTab === "products" ? `共 ${visibleProducts.length} 个产品` : activeTab === "domains" ? `共 ${visibleDomains.length} 个品类` : activeTab === "mss-domains" ? `共 ${mssDomains.length} 个MSS领域` : activeTab === "dictionaries" ? `共 ${dictTotal} 个配置项` : activeTab === "users" ? `共 ${visibleUsers.length} 个用户` : `共 ${visibleOrganizations.length} 个区域 / ${officeTotal}个代表处`}</span></div>
  ...
  <input style={{gridColumn: '2 / span 2'}} value={sku.description} onChange={(event) => updateSku(index, "description", event.target.value)} placeholder="产品描述（配置/规格说明，可选）" aria-label={`第${index + 1}个产品描述`} />
    ...
    {organizationModal && <Dialog title={organizationModal.type === "region" ? `${organizationModal.id === "new" ? "新增" : "编辑"}区域` : `${organizationModal.id === "new" ? "新增" : "编辑"}代表处`} description={organizationModal.type === "region" ? "可先创建区域，再到用户管理创建接口人并分配该区域" : `所属区域：${organizations.find((item) => item.id === organizationModal.regionId)?.name}；可先创建代表处再分配接口人`} onClose={() => setOrganizationModal(null)} footer={<><button className="button button-secondary compact-button" type="button" onClick={() => setOrganizationModal(null)}>取消</button><button className="button button-primary compact-button" type="button" onClick={saveOrganization}>保存配置</button></>}><div className="dialog-form"><label>{organizationModal.type === "region" ? "区域名称" : "代表处名称"}<input value={organizationForm.name} onChange={(event) => setOrganizationForm((current) => ({ ...current, name: event.target.value }))} placeholder={organizationModal.type === "region" ? "例如 北美MKT" : "例如 美国代表处"} /></label><label>{organizationModal.type === "region" ? "区域接口人（可后配）" : "代表处接口人（可后配）"}<input value={organizationForm.owner} onChange={(event) => setOrganizationForm((current) => ({ ...current, owner: event.target.value }))} placeholder={organizationModal.type === "region" ? "可选；创建接口人时分配本区域" : "可选；创建接口人时分配本代表处"} /></label>{!organizationForm.owner.trim() && <p className="config-hint">保存后该{organizationModal.type === "region" ? "区域" : "代表处"}显示“待配置”；创建区域/代表处接口人并选择本{organizationModal.type === "region" ? "区域" : "代表处"}后将自动回填。</p>}{organizationModal.type === "office" && <label>覆盖国家/地区<input value={organizationForm.countries} onChange={(event) => setOrganizationForm((current) => ({ ...current, countries: event.target.value }))} placeholder="多个国家用顿号或逗号分隔" /></label>}{organizationError && <p className="config-error"><IconAlertTriangleFilled size={16} />{organizationError}</p>}</div></Dialog>}
    {editingDictItem && <Dialog title={editingDictItem === "new" ? `新增${DICT_TYPE_MAP[activeDictType].name}` : `编辑${DICT_TYPE_MAP[dictItemForm.dictType].name}`} description={DICT_TYPE_MAP[dictItemForm.dictType].description} onClose={() => setEditingDictItem(null)} footer={<><button className="button button-secondary compact-button" type="button" onClick={() => setEditingDictItem(null)}>取消</button><button className="button button-primary compact-button" type="button" onClick={saveDictItem}>{editingDictItem === "new" ? "创建配置项" : "保存更新"}</button></>}><div className="dialog-form"><label>所属类型<select value={dictItemForm.dictType} onChange={(event) => setDictItemForm((current) => ({ ...current, dictType: event.target.value }))} disabled={editingDictItem !== "new"}>{Object.entries(DICT_TYPE_MAP).map(([type, config]) => <option key={type} value={type}>{config.name}</option>)}</select></label><div className="two-field-row"><label>编码<sup>*</sup><input value={dictItemForm.code} onChange={(event) => setDictItemForm((current) => ({ ...current, code: event.target.value }))} placeholder="例如 V3" disabled={editingDictItem !== "new"} /></label><label>排序<input type="number" value={dictItemForm.sortOrder} onChange={(event) => setDictItemForm((current) => ({ ...current, sortOrder: Number(event.target.value) }))} placeholder="数字越小越靠前" /></label></div><label>显示名称<sup>*</sup><input value={dictItemForm.name} onChange={(event) => setDictItemForm((current) => ({ ...current, name: event.target.value }))} placeholder="例如 V3" /></label><label>说明（可选）<input value={dictItemForm.description} onChange={(event) => setDictItemForm((current) => ({ ...current, description: event.target.value }))} placeholder="对该配置项的补充说明" /></label>{dictItemError && <p className="config-error"><IconAlertTriangleFilled size={16} />{dictItemError}</p>}</div></Dialog>}
