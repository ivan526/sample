...
export function ConfigurationPage({ products, domains, organizations, dictionaries = {}, onAddProduct, onUpdateProduct, onAddDomain, onUpdateDomain, onAddOrganization, onUpdateOrganization, onAddDictionaryItem, onUpdateDictionaryItem, onDeleteDictionaryItem }) {
  ...
  const saveDomain = () => {
    if (!domainForm.name.trim() || !domainForm.gtm.trim() || !domainForm.stockingOwner.trim()) { setDomainError("请完整填写领域名称、GTM接口人和领域备货接口人。"); return; }
    const payload = { ...domainForm, name: domainForm.name.trim(), gtm: domainForm.gtm.trim(), stockingOwner: domainForm.stockingOwner.trim() };
  ...
  const tabAction = activeTab === "products" ? { label: "新增产品", onClick: () => openProduct(null) }
    ...
    : { label: "新增字典项", onClick: () => openDictItem(null) };
  ...
  return <main className="workspace workspace-no-footer">
    <PageHeader title="配置管理" description="维护产品领域、责任人、组织架构及基础枚举数据，确保需求、执行和库存使用同一口径" action={<button className="button button-primary compact-button add-product-button" type="button" onClick={tabAction.onClick}><IconPlus size={18} />{tabAction.label}</button>} />
    <MetricStrip items={[{ label: "启用产品", value: products.filter((item) => item.enabled).length, unit: "个", hint: `${products.reduce((sum, item) => sum + item.skus.length, 0)}个SKU`, icon: IconBoxMultiple }, { label: "产品领域", value: domains.filter((item) => item.enabled).length, unit: "个", hint: "按领域归属GTM", icon: IconSettings }, { label: "基础数据项", value: dictTotal, unit: "项", hint: "样机阶段/领域/需求依据等枚举", icon: IconListDetails }, { label: "区域 / 代表处", value: `${organizations.length}/${officeTotal}`, unit: "区/处", hint: `覆盖${countryTotal}个国家/地区`, icon: IconWorld }]} />
    <div className="config-tabs" role="tablist" aria-label="配置类型">{[["products", "产品配置", products.length], ["domains", "领域与责任人", domains.length], ["dictionaries", "基础数据", dictTotal], ["organizations", "区域与代表处", officeTotal]].map(([id, label, count]) => <button type="button" role="tab" aria-selected={activeTab === id} className={activeTab === id ? "config-tab-active" : ""} key={id} onClick={() => { setActiveTab(id); setQuery(""); }}>{label}<span>{count}</span></button>)}</div>
    <section className="ops-surface config-surface">
      <div className="surface-title"><div><h2>{activeTab === "products" ? "产品主数据" : activeTab === "domains" ? "领域与责任人" : activeTab === "dictionaries" ? "基础枚举数据" : "区域与代表处"}</h2><p>{activeTab === "products" ? "产品选择所属领域后，自动继承该领域的GTM和备货接口人" : activeTab === "domains" ? "同一产品领域由固定GTM和领域备货接口人负责" : activeTab === "dictionaries" ? "维护样机阶段、MSS领域、需求依据等系统枚举值，全平台统一使用" : "维护区域、代表处及国家/地区的组织层级和接口人"}</p></div><span className="surface-summary">{activeTab === "products" ? `共 ${visibleProducts.length} 个产品` : activeTab === "domains" ? `共 ${visibleDomains.length} 个领域` : activeTab === "dictionaries" ? `共 ${dictTotal} 个配置项` : `共 ${visibleOrganizations.length} 个区域 / ${officeTotal}个代表处`}</span></div>
      ...
      {activeTab === "products" && <div className="plain-table-wrap"><table className="plain-table config-table product-config-table"><thead><tr><th>产品名称</th><th>所属领域</th><th>样机阶段</th><th>样机提供时间</th><th>责任人</th><th>型号 / BOM</th><th>默认截止</th><th>状态</th><th>操作</th></tr></thead><tbody>{visibleProducts.map((item) => { const domain = domainFor(item); const missingBom = !item.skus.length || item.skus.some((sku) => !sku.bom); return <tr key={item.id}><td><strong>{item.name}</strong><small>{item.id}</small></td><td><span className="domain-chip">{domain?.name || "待配置"}</span></td><td>{item.stage}</td><td>{item.supply || "待产品线确认"}</td><td><strong>GTM · {domain?.gtm || "待配置"}</strong><small>备货 · {domain?.stockingOwner || "待配置"}</small></td><td><strong>{item.skus.length ? `${item.skus.length}个型号` : "产品立项阶段"}</strong><small className={missingBom ? "warning-text" : ""}>{missingBom ? "BOM待产品线补充" : item.skus.map((sku) => `${sku.sku} / ${sku.bom}`).join("、")}</small></td><td>{item.deadline || "待计划下发"}</td><td><StatusBadge>{item.enabled ? "启用中" : "已停用"}</StatusBadge></td><td><div className="config-actions"><button className="table-action" type="button" onClick={() => openProduct(item)}><IconPencil size={15} />编辑</button><button className="table-action muted-action" type="button" onClick={() => onUpdateProduct({ ...item, enabled: !item.enabled })}>{item.enabled ? "停用" : "启用"}</button></div></td></tr>; })}{!visibleProducts.length && <tr><td className="empty-cell" colSpan="9">未找到匹配的产品配置</td></tr>}</tbody></table></div>}
      {activeTab === "domains" && <div className="plain-table-wrap"><table className="plain-table config-table domain-config-table"><thead><tr><th>产品领域</th><th>领域说明</th><th>GTM接口人</th><th>领域备货接口人</th><th>关联产品</th><th>状态</th><th>操作</th></tr></thead><tbody>{visibleDomains.map((item) => { const linked = products.filter((product) => product.categoryId === item.id); return <tr key={item.id}><td><strong>{item.name}</strong><small>{item.id}</small></td><td>{item.description || "—"}</td><td><span className="owner-cell"><i>{item.gtm.slice(0, 1)}</i><span><strong>{item.gtm}</strong><small>GTM负责人</small></span></span></td><td><span className="owner-cell"><i>{item.stockingOwner.slice(0, 1)}</i><span><strong>{item.stockingOwner}</strong><small>备货执行衔接</small></span></span></td><td><strong>{linked.length}个产品</strong><small>{linked.map((product) => product.name).join("、") || "暂无关联"}</small></td><td><StatusBadge>{item.enabled ? "启用中" : "已停用"}</StatusBadge></td><td><div className="config-actions"><button className="table-action" type="button" onClick={() => openDomain(item)}><IconPencil size={15} />编辑</button><button className="table-action muted-action" type="button" onClick={() => onUpdateDomain({ ...item, enabled: !item.enabled })}>{item.enabled ? "停用" : "启用"}</button></div></td></tr>; })}{!visibleDomains.length && <tr><td className="empty-cell" colSpan="7">未找到匹配的领域配置</td></tr>}</tbody></table></div>}
      {activeTab === "organizations" && <div className="plain-table-wrap"><table className="plain-table config-table organization-config-table"><thead><tr><th>组织层级</th><th>接口人</th><th>覆盖国家/地区</th><th>下级数量</th><th>状态</th><th>操作</th></tr></thead><tbody>{visibleOrganizations.map((region) => { const open = Boolean(expandedRegions[region.id]); return <OrganizationRows key={region.id} region={region} open={open} onToggle={() => setExpandedRegions((current) => ({ ...current, [region.id]: !current[region.id] }))} onEditRegion={() => openRegion(region)} onAddOffice={() => openOffice(region, null)} onEditOffice={(office) => openOffice(region, office)} />; })}{!visibleOrganizations.length && <tr><td className="empty-cell" colSpan="6">未找到匹配的区域或代表处配置</td></tr>}</tbody></table></div>}
