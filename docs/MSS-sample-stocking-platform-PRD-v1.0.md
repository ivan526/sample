...
创建后状态为“待下发”。若产品仅有名称、暂无SKU/BOM，允许创建和下发；页面提示“BOM可后补，不影响先发起收集”。同一产品允许多轮计划，但同一时间只能有一条未结束计划，除非GTM明确确认并行批次。
...
产品行辅助信息展示产品领域、GTM、备货接口人、样机阶段和SKU数量。SKU行展示BOM。进度单元格使用“申/发”双进度条。
...
页面标题为“配置管理”，包含三个页签：产品配置、领域与责任人、区域与代表处。三个页签共用搜索、启停状态、表格视觉和新增/编辑弹窗。
...
| 所属领域 | 是 | 决定继承的GTM和领域备货接口人 |
| 当前样机阶段 | 否 | EVT/DVT/VN2/MP等可配置枚举 |
| 预计样机提供时间 | 否 | 可填“待产品线确认” |
| 计划默认截止 | 否 | 新建计划时作为默认值，可覆盖 |
| 默认收集范围 | 否 | 全部区域或预设区域集合 |
| SKU/产品型号 | 否 | 一对多，可后补 |
| BOM编码 | 否 | 与SKU关联，可后补 |
| 启用状态 | 是 | 停用产品不允许新建计划，但保留历史数据 |

产品不直接保存GTM和备货接口人姓名，页面通过所属领域实时解析。修改领域责任人后，产品配置、需求收集、执行情况中的责任人必须同步更新。
