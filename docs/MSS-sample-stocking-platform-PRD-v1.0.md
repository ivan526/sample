...
| 收集中 | 至少一个领域未完成反馈，或已反馈/已导出数据因批准变更而回退 | 领域下发区域；区域填报；MSS跟进/代录/反馈 | 待GTM收口 |
...
区域提交后的修改采用分阶段规则：截止前且领域未反馈时，区域接口人可自主撤回；截止后、领域已反馈或计划已导出时，区域接口人提交变更申请，由本MSS领域接口人审批。审批通过后区域和领域进度回退，原提交快照保留；若领域已经反馈，原反馈转入失效历史；若计划已经导出，原导出文件保留，重新收集完成后生成下一导出版本。
...
本期必须完整实现第1、2、4、7项；其他导航保留高保真入口和可扩展路由。
...
列表默认按最近更新时间倒序，支持按最近更新、创建时间、截止时间、需求总量、收集进度升降序排列；支持状态筛选、关键词搜索和“包含已归档”。采用服务端分页，默认每页20条，可切换20/50/100条。搜索、筛选、排序或每页条数变化时回到第1页；从计划详情返回时保留此前查询条件和页码。

GTM操作规则：

- 产品建档：显示“完善产品”。
- 待下发：显示“下发全部领域”。
- 待下发且尚未生成领域任务：允许“删除”，删除前二次确认；删除后不可恢复。
- 收集中/待领域反馈：显示“查看收集进度”。
- 待GTM收口：显示“查看领域反馈”“导出排产”。
- 已导出：显示“查看领域反馈”“重新导出”。
- 已下发但所有区域均未开始填报：允许“取消”；取消后保留计划、领域任务和审计记录，领域/区域接口人不再看到可执行任务。
- 已产生草稿、提交、反馈或执行数据：不允许删除或取消，只允许“归档”；归档计划默认隐藏，可通过“包含已归档”查看。
- 永远不显示“查看填报”“模拟收齐”。
...
- 每条任务显示产品、GTM/MSS下发信息、覆盖组织、截止时间、当前状态和“进入填报/查看与变更”。
- 状态至少区分“待填报、修改中、已提交Vn、变更审批中”；若被MSS退回或变更获批，则恢复可编辑并展示原因。
...
1. 截止前且领域未反馈：区域接口人点击“撤回修改”，填写原因后立即回到“修改中”。
2. 截止后、领域已反馈或计划已导出：按钮显示“申请变更”，填写原因后进入“变更审批中”，页面继续只读。
...
导出仅允许“待GTM收口/已导出”状态，且计划下不能存在待审批变更。文件至少包含：计划编号、导出版本、产品、产品领域、SKU/产品项、BOM编码、MSS区域、代表处、国家/地区、确认需求数量、需求依据、计划使用时间、备注、领域反馈人、反馈时间。

导出后保存文件名、导出人、导出时间、计划版本和数据快照。文件名包含`V1/V2…`；导出后批准变更时，旧文件和旧反馈历史不能被覆盖，新一轮收集完成后导出下一版本。
...
自动匹配使用“MSS领域+BOM编码+区域+代表处+国家”组合口径。业务领域支持按MSS领域名称或编码标准化；对TSMP常见的“GTM样机/MKT样机”和平台“GTM领域/MKT领域”，匹配时仅去除末尾“样机、领域、业务领域”后缀后比较，不使用模糊包含匹配。BOM编码用于定位产品型号；地区部、代表处、国家必须满足组织树父子关系。确认需求暂未细分到国家时，允许同一代表处下的有效国家匹配该代表处需求。
...
整文件导入指纹包含匹配规则版本。匹配规则升级后，原“待维护映射/未匹配”文件允许原样重新执行；此前已匹配成功的行仍通过行级指纹识别为重复，不得再次累计。

异常分类：

- 待维护映射：原始值可识别类型，但未命中主数据。失败明细必须显示Excel原始值、当前已启用的候选配置和系统已尝试的标准化规则。
...
采用树形融合表，默认仅展示产品，并按“产品→SKU→MSS领域→区域→代表处→国家/地区”逐层展开。所有层级共用同一组指标列，避免把六个组织维度横向铺开。字段固定为：

1. 产品/SKU/领域/区域组织（树形列）
2. 确认需求
3. 已备货
4. 累计申请
5. 累计发货
...
父级节点聚合所有下级数据，展开后查看下一级；SKU节点辅助展示BOM。组织维度为空的排产或库存事实显示为“全局”，避免把全局库存错误复制到每个区域。进度单元格使用“申/发”双进度条；区域、代表处、国家筛选继续对整棵树生效。导出仍使用全部维度平铺明细。
...
基础数据只维护样机阶段和需求依据。样机阶段固定为`V3`、`V4`、`VN1`、`VN2`；MSS业务领域在独立的“MSS业务领域”页签维护，不在基础枚举中重复配置。
...
- 区域字段：名称、区域接口人、启用状态。
- 区域和代表处均可先创建后配置接口人；未配置时显示“待配置”。创建区域/代表处接口人并选择对应区域或代表处后，组织节点负责人自动同步。
...
- 删除区域时，未被计划、需求或执行数据引用的区域树直接物理删除；已有业务引用的区域树整体停用并退出新业务选择，稳定ID与历史快照继续保留。
...
| 计划 | `GET /collection/plans` | 按角色与范围获取计划/任务列表；GTM列表支持分页、排序、筛选和包含归档 |
| 计划 | `POST /collection/plans` | GTM新建计划 |
| 计划 | `DELETE /collection/plans/{id}` | 删除未下发且无领域任务的计划 |
| 计划 | `POST /collection/plans/{id}/cancel` | 取消已下发但区域尚未开始填报的计划 |
| 计划 | `POST /collection/plans/{id}/archive` | 归档计划并保留全部历史数据 |
...
- 已提交需求在界面和API均为只读，只有截止前自主撤回、MSS退回或变更审批通过后才恢复编辑；产品级临时需求在补充SKU后仍保留为“待分配到正式SKU”，不得静默丢失。
...
9. 仅“未下发且未生成领域任务”的收集计划允许物理删除；其余业务数据使用取消、归档或停用，历史计划、需求和执行数据不可丢失。
