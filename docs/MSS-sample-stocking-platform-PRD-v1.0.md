...
5. TSMP导出的发货数据需要按照产品型号、发货区域、代表处与已确认需求匹配。
...
| 收集计划 | GTM面向指定领域/区域发起的一轮需求收集，独立于产品主数据 |
...
| 待下发 | 计划已创建、范围和截止时间已确认 | GTM下发计划 | 收集中 |
| 收集中 | 至少一个区域未提交 | 区域保存/提交；MSS跟进/代录 | 待领域反馈 |
| 待领域反馈 | 计划范围内所有区域已提交 | MSS检查汇总并正式反馈 | 待GTM收口 |
| 待GTM收口 | MSS已正式反馈GTM | GTM查看反馈、导出排产 | 已导出 |
| 已导出 | GTM已生成排产文件 | GTM重新导出；查看历史 | 已导出 |

禁止跳转：`收集中 → 待GTM收口`、`待下发 → 待领域反馈`。所有状态变更记录操作者、时间、前后状态和备注。
...
| DC-03 | MSS领域任务列表 | MSS领域接口人 | 需求收集 · 我的领域任务 |
...
| 待区域反馈 | 进行中计划的`计划区域数-已提交区域数`之和 |
| 已收集需求 | 当前筛选范围内已形成的需求数量合计 |
| BOM待补充 | 启用产品中无SKU或至少一个SKU无BOM的产品数 |
| 覆盖组织 | 启用区域数及代表处合计 |

计划表字段：计划/产品、下发范围、收集进度、需求汇总、BOM准备度、当前节点、截止时间、GTM操作。
...
| MSS业务领域 | 单选 | 是 | 从MSS业务领域配置取值；决定计划下发、领域反馈和数据权限 |
| 下发范围 | 单选/多选 | 是 | 全部MSS区域或指定区域；至少选择1个区域 |
| 收集截止时间 | 日期时间 | 是 | 必须晚于当前时间 |
| 计划说明 | 文本 | 否 | 最多500字 |

创建后状态为“待下发”。若产品仅有名称、暂无SKU/BOM，允许创建和下发；页面提示“BOM可后补，不影响先发起收集”。同一产品可按不同样机阶段、不同MSS业务领域分别发起收集；产品、样机阶段、MSS业务领域三者相同时，只能有一条未结束计划。

### 6.4 DC-03 MSS领域任务列表

目标：领域接口人只看到自己领域内已下发的任务。

页面顶部明确显示“领域名 · 接口人”。流程条为：接收GTM计划→组织区域收集→检查领域汇总→反馈给GTM。

列表字段：计划/产品、GTM下发信息、区域进度、领域需求、领域任务状态、截止时间、下一步。
...
2. 标准化发货区域。
3. 标准化代表处。
4. 国家/地区作为增强维度，不作为高保真当前版本的强制主键。

导入过程：上传→表头校验→字段映射→数据标准化→重复检测→自动匹配→统计结果→写入累计执行数据。

必需导入字段：产品型号、发货区域、代表处、发货数量；推荐字段：BOM、国家/地区、发货日期、TSMP申请单号、申请人、数据唯一键。

去重规则：优先使用TSMP数据唯一键；无唯一键时使用`申请单号+SKU+组织+发货日期+数量`生成指纹。重复行不得重复累计。
...
产品不保存样机阶段、MSS业务领域或默认收集范围，也不直接保存GTM和备货接口人姓名。样机阶段、MSS业务领域和收集范围均在新建收集计划时选择；责任人由页面通过所属产品品类实时解析。修改品类责任人后，产品配置、需求收集、执行情况中的责任人必须同步更新。
...
- 系统管理员不展示负责范围选择。
- 用户列表展示已配置的负责范围；创建或调整责任人后，产品品类、MSS领域及角色数据权限同步生效。
...
| 计划 | `POST /collection/plans/{id}/release` | GTM下发计划 |
...
- 产品品类绑定GTM与备货接口人；MSS业务领域在收集计划中选择并绑定MSS领域接口人，两个责任模型不得混用。
...
- [ ] GTM计划页不出现“查看填报”“模拟收齐”。
- [ ] 下发后MSS领域接口人看到任务，区域接口人看到本区域填报任务。
