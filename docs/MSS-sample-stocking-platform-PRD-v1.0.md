...
创建后状态为“待下发”。若产品仅有名称、暂无SKU/BOM，允许创建和下发；页面提示“BOM可后补，不影响先发起收集”。同一产品可按不同样机阶段分别发起收集；同一产品、同一样机阶段同时只能有一条未结束计划。
...
产品不保存样机阶段，也不直接保存GTM和备货接口人姓名。样机阶段在新建收集计划时选择；责任人由页面通过所属领域实时解析。修改领域责任人后，产品配置、需求收集、执行情况中的责任人必须同步更新。
