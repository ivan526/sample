...
这套数据用于本地开发、产品演示和业务验收。全新开发库会自动初始化；已有开发库执行`npm run data:load:huawei`即可增量补齐。命令可重复执行，会将固定测试计划恢复到标准状态，但不删除自行录入的产品、用户和计划。
...
| 区域接口人 | lateowner | 机关区域接口人 | 机关区域；MKT、服务领域 | 后配置账号SKU显示、跨领域上下文保持 |
...
- 后配置的机关区域接口人可看到同一产品的MKT、服务两个领域任务及准确SKU范围。
