...
export const inventoryRepository = {
  async list(filters: { productId: string; keyword: string; onlyDiff: boolean }) {
  ...
  async check(id: string, input: { actualQuantity: number; reason: string; version: number }, userId: string) {
    ...
    try {
      ...
      `, [input.actualQuantity, available, input.reason, userId, id, input.version]);
      ...
      await client.query(`INSERT INTO audit_log (id, actor_id, action, entity_type, entity_id, before_data, after_data)
        VALUES ($1, $2, 'CHECK_INVENTORY', 'INVENTORY_BALANCE', $3, $4, $5)`, [crypto.randomUUID(), userId, id, JSON.stringify(before), JSON.stringify({ actualQuantity: input.actualQuantity, availableQuantity: available, reason: input.reason })]);
